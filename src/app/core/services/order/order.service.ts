...
export class OrderService {
   ...
   */
  private sendRedirectUrlsIfRequired(order: Order, currentLocale: Locale): Observable<Order> {
    if (
      order.orderCreation &&
      ...
      const body = {
        orderCreation: {
          redirect: {
            cancelUrl: `${location.origin}/checkout/payment;lang=${currentLocale.lang}?redirect=cancel&orderId=${order.id}`,
            failureUrl: `${location.origin}/checkout/payment;lang=${currentLocale.lang}?redirect=failure&orderId=${order.id}`,
            successUrl: `${location.origin}/checkout/receipt;lang=${currentLocale.lang}?redirect=success&orderId=${order.id}`,
          },
          status: 'CONTINUE',
        },
      };
